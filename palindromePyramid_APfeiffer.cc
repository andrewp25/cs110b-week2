...
specified by the user. */
    ...
    if (pyramidRows == 1)
    {
        cout << pyramidRows << endl;	// To print out the 1 if the user enters 1.
    }
    ...
    if (pyramidRows > 1 && pyramidRows <= 7)
        {
            ...
            for (counter = 1; counter < pyramidRows; counter++)
            {
                ...
                // Large integer type to hold the base 11 powers that are multiplied by themselves.
                long result  = base * base;

                /* Large base 11 powered numbers are printed through each loop iteration until loop
                counter reaches end. */
